...
## 9.9 90度交差の選好（原則3）と、街路の散らかり／疎密の作り分け（2026-07-05）

理想の5原則（`docs`外のメモリ`project-road-ideal`参照）のうち、ユーザー指定で原則3（90度交差）と原則1/2（疎密の作り分け）に着手。size_weight導入で幹線道路の団子は消えたが、次は**建設コストの低い街路が無秩序に増える**問題が残っていた。

**(A) 角度罰則を90度格子ベースに作り替え（原則3）**: `get_angle_penalty` を「gapが `min_angle`(45°)未満なら罰する」から「gapが90度格子 {90°,180°,270°} からどれだけずれているか `min(|g-90|,|g-180|,|g-270|)` を罰する」に変更。直進(180)・直角カーブ(90/270)・T字(90/90/180)・十字(90×4)は無罰、鋭角(→0°)や斜め(45°,135°)を罰する。`angle_deadzone`(格子まわりの許容幅、デフォルト10°)を導入。引数は `min_angle` を廃止し `angle_deadzone` に。連続損失に加算（勾配で道路の向きを90度方向へ動かす）。

**(B) 街路の連続コストを上げる試み → 却下（崩壊）**: 街路の連続建設コストを上げれば疎密が作れると考え、連続コストの幅指数 `size_width_exponent` を離散の `cost_width_exponent`(2.5)から分離して1.0〜2.0に下げる案を試した（街路幅0.02のコストを底上げ）。しかし**道路網が種(1エッジ)のまま崩壊**した。原因: 到達半径sigmaが小さく道路1本あたりの密度カバレッジ利得が小さい(≈3.6e-4)ため、街路コストがそれを超えると「有用な街路の追加すら損失増」となり全成長提案が却下される。→ `size_width_exponent` は離散と同じ2.5、`size_weight`=20に据え置き。

**結論: 街路の散らかり／疎密の作り分けは、街路コストではなく (A)の90度罰則が実効的に担う。** 検証（Donut, 200step, size128）:

| 設定 | edges | coverage | 90度からの平均ずれ | 街路の低人口域への漏れ |
|---|---|---|---|---|
| 角度罰則OFF | 136 | 52% | 37° | 64% |
| **角度罰則ON (angle_weight=0.03〜0.05)** | 63 | 38% | **10°** | **11%** |

90度罰則で交差が直交化するだけでなく、街路が直進・直交して走るため**低人口域への無秩序な蛇行が消え(64%→11%)、疎密の作り分けも改善**した（原則1/2にも寄与）。coverageは200stepでは下がるが本番5000stepで回復見込み。`angle_weight`は新罰則が旧より強い(gap二乗が最大~2)ため 0.02→0.03 に。`cost_width_exponent`デフォルトも2.0→2.5に統一。

**未対応**: 原則3をさらに徹底するなら「全道路を共通のグリッド方位へ揃える」大域整列（原則5=街区面積均一にも寄与）が考えられるが、per-node局所の90度選好で今回は十分と判断。原則4(行き止まり)・5(街区均一)は未着手。

## 9.10 中央の街路スクリブル → cleanup で密集地帯を検出して間引く（2026-07-05）

実データ(矩形ターゲット)で「中央に街路(細い白線が重なって塊に見える)がスクリブル状に過密化する」問題。原点中心の矩形で切り分けたところ、**当初の cleanup(交差解消)が主因**（中央ノード密度が交差解消ONで約2.6倍、OFFなら全半径帯でほぼ均一）と判明。高密度域では街路が多数交差し、交差を分割するたびに短エッジが増えて更に交差…と中央だけ断片化が暴走していた。

ユーザー指示: 「cleanupで密集地帯を検出し、交差点やそれに接続する道路を間引け」。実装:
- **交差解消をデフォルトOFF**(`cleanup_resolve_crossings=False`)。過密化の主因なので止める。交差点は損失駆動のSnapで必要な箇所に作られる。
- **`RoadNetwork.decimate_dense(cell_size, max_per_cell, highway_width)` を新設**(`RoadTask.cleanup`から毎回呼ぶ)。`cell_size`グリッドで1セルのノード数が`max_per_cell`を超える密集セルを検出し、そのセル内の**街路ノード(幹線に接しないノード)を1つの代表ノードへ統合(collapse)**、自己ループ除去・重複エッジは幅最大(=幹線優先)で集約。
  - 統合は連結性を壊さない(外部接続は代表へ移る、不変条件A)。
  - **幹線ノード・エッジはremap対象外・自己ループ化しない**ので幹線サブグラフの連結性・非空性を完全保持(不変条件B)。
  - デフォルト `decimate_cell_size=0.06`, `decimate_max_per_cell=2`, `decimate_dense=True`。

**検証**: 単体で密集9ノード→3ノードに間引き(連結・幹線保持)。大規模合成スクリブル(中央247ノード)で cell0.06/max2 → 中央42ノード(約1/6)、cell0.10/max1 → 中央18ノード(約1/14)、いずれも連結性・幹線サブグラフ保持。**注意: 小規模(〜150step, 中央十数ノード)ではセルあたり閾値を超えず decimation は発火しない(ON/OFF同一)。ユーザーの実規模(数百エッジ)のスクリブルで初めて効く**。実ループでは cleanup毎(10step毎)に発火し大規模化前に抑える。

**教訓**: Repairabilityのための交差解消は、高密度域では断片化の暴走源になり得る。密度過多の解消は「交差を分割して増やす」方向ではなく「密集を検出して統合し減らす」方向(decimation)が正しい。decimationはノード統合ベースなら連結性・幹線階層を自動的に保てる。
